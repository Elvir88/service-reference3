<template>
  <div>
    <h2 id="page-heading" data-cy="PromotionOnLocationHeading">
      <span v-text="$t('serviceReference3App.promotionOnLocation.home.title')" id="promotion-on-location-heading"
        >Promotion On Locations</span
      >
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('serviceReference3App.promotionOnLocation.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PromotionOnLocationCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('serviceReference3App.promotionOnLocation.home.createLabel')"> Create a new Promotion On Location </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && promotionOnLocations && promotionOnLocations.length === 0">
      <span v-text="$t('serviceReference3App.promotionOnLocation.home.notFound')">No promotionOnLocations found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('title')">
              <span v-text="$t('serviceReference3App.promotionOnLocation.title')">Title</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('dateFrom')">
              <span v-text="$t('serviceReference3App.promotionOnLocation.dateFrom')">Date From</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="serviceReference3App.promotionOnLocation.delete.question"
      ...
      <div class="modal-body">
        <p
          ...
          v-text="$t('serviceReference3App.promotionOnLocation.delete.question', { id: removeId })"
        >

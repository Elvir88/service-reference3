<template>
  <div>
    <h2 id="page-heading" data-cy="ServiceInPacketDiscountHeading">
      <span v-text="$t('serviceReference3App.serviceInPacketDiscount.home.title')" id="service-in-packet-discount-heading"
        >Service In Packet Discounts</span
      >
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('serviceReference3App.serviceInPacketDiscount.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ServiceInPacketDiscountCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('serviceReference3App.serviceInPacketDiscount.home.createLabel')">
    ...
    <div class="alert alert-warning" v-if="!isFetching && serviceInPacketDiscounts && serviceInPacketDiscounts.length === 0">
      <span v-text="$t('serviceReference3App.serviceInPacketDiscount.home.notFound')">No serviceInPacketDiscounts found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="serviceInPacketDiscounts">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('serviceReference3App.serviceInPacketDiscount.coefficient')">Coefficient</span></th>
            <th scope="row"><span v-text="$t('serviceReference3App.serviceInPacketDiscount.service')">Service</span></th>
            <th scope="row"><span v-text="$t('serviceReference3App.serviceInPacketDiscount.tariff')">Tariff</span></th>
            <th scope="row"><span v-text="$t('serviceReference3App.serviceInPacketDiscount.packetDiscount')">Packet Discount</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="serviceReference3App.serviceInPacketDiscount.delete.question"
      ...
      <div class="modal-body">
        <p
          ...
          v-text="$t('serviceReference3App.serviceInPacketDiscount.delete.question', { id: removeId })"
        >
